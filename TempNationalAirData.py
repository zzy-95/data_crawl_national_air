...
def getData(link):
    ...
    while flag:
        try:
            ...
            df.replace('','NaN',inplace = True)# replace empty value as NaN
            df.replace(' - ','NaN',inplace = True)

            df.columns = ['WatchPoint','AQI','Level','PrimaryPollution','PM2.5','PM10','CO','NO2','O3_1h','O3_8h','SO2']#change head
            ...
            csvName = 'data/' + cityname + '.csv'# file name

            if os.path.exists(csvName)==True:     # write csv with tablehead if no existing file
                df.to_csv(csvName, index = False, mode = 'a+',header = False)
            else:
                df.to_csv(csvName,index = False, mode = 'a+')
...
#engine = create_engine('mysql+pymysql://root:123456@localhost:port/weather', encoding = 'utf-8')
while True:
    print('Start getting data at ',datetime.now())
    start = datetime.now()
    for i in range(len(linklist)):
        try:
            #pd.io.sql.to_sql((),environment,con = engine, if_exists = 'append', index = False)
            getData(linklist[i])
        except:
